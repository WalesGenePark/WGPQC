...
if (!-e $bamFile) {
	run("samtools view -bt $ref.fasta.fai $samFile | samtools sort - $sample ; samtools index $bamFile");
	}

# mark duplicates
 if (!-e $markdupBamFile) {
                run("java -jar /software/picard-tools/MarkDuplicates.jar INPUT=$bamFile OUTPUT=$markdupBamFile METRICS_FILE=markdup.log VALIDATION_STRINGENCY=LENIENT ASSUME_SORTED=true");
                }
