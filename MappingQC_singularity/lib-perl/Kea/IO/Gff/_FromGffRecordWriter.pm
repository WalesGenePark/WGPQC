...
# CLASS NAME
package Kea::IO::Gff::_FromGffRecordWriter;
use Kea::Object;
use Kea::IO::IWriter;
...
sub write {
	...
	my $self 		= shift;
	my $FILEHANDLE	= $self->check(shift);
	my $gffRecord 	= $self->{_gffRecord};
	
	my $text = "##gff-version\t3\n";
	$text .= "##feature-ontology\t" . $gffRecord->getFeatureOntologyURI . "\n";
	$text .= "##attribute-ontology\t" . $gffRecord->getAttributeOntologyURI . "\n";
...
sub writeWithSequence {

	my $self 		= shift;
	my $FILEHANDLE	= $self->check(shift);
	my $gffRecord 	= $self->{_gffRecord};
	
	# First print tabular data.
	#===========================================================================
