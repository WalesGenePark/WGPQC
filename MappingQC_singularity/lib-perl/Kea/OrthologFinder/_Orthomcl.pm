...
# CLASS NAME
package Kea::OrthologFinder::_Orthomcl;
use Kea::Object;
...
BEGIN {
	...
	if (!-e $_orthomclPath || !-d $_orthomclPath) {
		confess "\nERROR: Problem with $_orthomclPath.\n\n";
		}
...
my $createProteinFastaFile = sub{
	...
	my $outfile = $record->getPrimaryAccession . ".fas";
	
	print ">>> Write $outfile...\n";
	open (OUT, ">$dataDirectory/$outfile") or die "\nERROR: Could not write to $outfile.";
...
sub run {
	...
	my $recordCollection =
		$self->check($args{-records}, "Kea::IO::RecordCollection");

	my $outfile = $args{-outfile};

	$self->{_recordCollection} = $recordCollection;
	...
	# Create.
	my $TEMPLATE = $_TEMPLATE; # Prevents possible problems if more than one instance of class.
	...
	for (my $i = 0; $i < $recordCollection->getSize; $i++) {
		push(
			@files,
			$self->$createProteinFastaFile($dataDirectory, $recordCollection->get($i))
			);
		}
	...
	# Run perl script.
	my $command = "$_orthomclPath/orthomcl.pl --mode $mode --fa_files \"$fileString\"";
	...
	open(IN, "$WORKING_DIR/all_orthomcl.out") or
		$self->throw("Could not open $WORKING_DIR/all_orthomcl.out");
	...
	system ("rm -rvf $WORKING_DIR"); 												# TODO - replace with more portable method.
	system ("rm -rvf $DATA_DIR");													# TODO - replace with more portable method.
	unlink("formatdb.log");
	#===========================================================================
...
sub getClustersAsFeatureCollections {
	...
	foreach my $cluster (@clusters) {
		...
		my $featureCollection = Kea::IO::Feature::FeatureCollection->new("cluster_" . $cluster->getId);
