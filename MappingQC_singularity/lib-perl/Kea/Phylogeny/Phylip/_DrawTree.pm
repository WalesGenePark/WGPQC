...
# CLASS NAME
package Kea::Phylogeny::Phylip::_DrawTree;
use Kea::Object;
use Kea::Phylogeny::Phylip::IPhylip;
our @ISA = qw(Kea::Object Kea::Phylogeny::Phylip::IPhylip);

## Purpose		: Private class for implementing drawtree - accessed via factory class.
...
use Kea::ThirdParty::Phylip::_DrawTreeWrapper;

use constant TMP_TREEFILE => "TMP_treefile.tre";
use constant TMP_PLOTFILE => "TMP_plotfile.ps";
use constant TMP_FONTFILE => "TMP_fontfile";
...
sub run {
	...
	my $tree	= $self->check($args{-tree}, "Kea::Phylogeny::ITree"); 
	my $OUT 	= $self->check($args{-out});
	...
	# Create tmp tree
	open (TREE, ">" . TMP_TREEFILE) or $self->throw("Could not create tmp file.");
	print TREE $tree->toString;
	close (TREE) or $self->throw("Could not close tmp file.");
	
	# Create fontfile.
	open (FONT, ">" . TMP_FONTFILE) or $self->throw("Could not create tmp file.");
	print FONT $_font1;
	close (FONT) or $self->throw("Could not close tmp file.");
	...
	# Run drawtree.
	my $drawtree = Kea::ThirdParty::Phylip::_DrawTreeWrapper->new;
	$drawtree->run(
		-infile 			=> TMP_TREEFILE,
		-outfile 			=> TMP_PLOTFILE,
	...
	# Read tmp plotfile to out stream.
	open (TMP, TMP_PLOTFILE) or $self->throw("Could not open tmp file.");
	while (<TMP>) {
		print $OUT $_;
		}
